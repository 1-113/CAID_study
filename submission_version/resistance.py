...
def failure_recovery(faulty_nodes, chosen_nodes):
    ...
    for i in range(N):
        if i in faulty_nodes and i in chosen_nodes:
            C = [j for j in range(N) if j != i and j not in chosen_nodes and abs(degree[j] - degree[i]) < delta]

            C_prime = [j for j in C if abs(S[i] - S[j]) < sigma_tol * S_bar]
            if not C_prime:
                C_prime = C
            ...
            for j in range(N):
                for k in range(N):
                    x_star[j] += p[j][k] * x[k] + p[k][j] * x[k]
                x_star[j] /= degree[j]

            min_diff = float('inf')
            candidate = C_prime[0] if C_prime else -1
            for j in C_prime:
                if abs(x_star[i] - x_star[j]) < min_diff:
                    min_diff = abs(x_star[i] - x_star[j])
                    candidate = j
...
ax.set_ylim(0, y_max)
plt.axvline(x=20, linestyle='--', color='black', linewidth=0.6, dashes=(5, 2))
plt.xlabel("Iterations")
plt.ylabel("Player's strategy value ($x$)")
plt.title("BA-CAID induction simulation")
plt.legend()
plt.savefig(os.path.join(figure_dir, 'Fig_resistance_convergence.pdf'), format='pdf', dpi=500, bbox_inches='tight', pad_inches=0.1)
...
plt.legend()
plt.savefig(os.path.join(figure_dir, 'Fig_resistance_chosen_players.pdf'), format='pdf', dpi=500, bbox_inches='tight', pad_inches=0.1)

plt.figure(4, dpi=300)
for node in replaced_nodes:
    replaced_nodes_record = [x_record[i][node] for i in range(len(x_record))]
    plt.plot(replaced_nodes_record, label=f'Player {node}', linewidth=0.8)
...
plt.legend()
plt.savefig(os.path.join(figure_dir, 'Fig_resistance_replaced_players.pdf'), format='pdf', dpi=500, bbox_inches='tight', pad_inches=0.1)
...
plt.savefig(os.path.join(figure_dir, 'Fig_with_resistance_energy_cost.pdf'), format='pdf', dpi=500, bbox_inches='tight', pad_inches=0.1)
"""
