...
def iteration():
    for i in range(N):
        f[i] = 0
        for j in range(N):
            f[i] += payoff_matrix_CAIPD[0] * x[i] * x[j] + payoff_matrix_CAIPD[1] * (1 - x[i]) * x[j] + \
                    payoff_matrix_CAIPD[2] * x[i] * (1 - x[j]) + payoff_matrix_CAIPD[3] * (1 - x[i]) * (1 - x[j])
...
def are_elements_converged(array, target_value, tolerance=1e-5):
    """
